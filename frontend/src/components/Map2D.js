...
// --- SearchControl (No changes needed) ---
function SearchControl({ setView }) {
    ...
    <div className="leaflet-top leaflet-right">
      <div className="leaflet-control leaflet-bar">
        <form onSubmit={handleSearch} className="search-form">
          <input type="text" placeholder="Search for a location..." value={query} onChange={(e) => setQuery(e.target.value)} className="search-input" />
...
// --- UPDATED WeatherPopup Component ---
...
function WeatherPopup() {
  ...
  useMapEvents({
    click(e) {
      ...
      // --- THE CHANGE IS IN THIS URL ---
      const locationUrl = `https://nominatim.openstreetmap.org/reverse?format=json&lat=${e.latlng.lat}&lon=${e.latlng.lng}&accept-language=en`;
      const weatherUrl = `http://localhost:5000/api/weather?lat=${e.latlng.lat}&lon=${e.latlng.lng}`;

      Promise.all([
        axios.get(locationUrl), // Fetch location in English
      ...
      .then(([locationRes, weatherRes]) => {
        ...
        setPopupData({ location: locationName, weather });
      })
      .catch(error => {
        ...
        popupData && (
          <div>
            {popupData.error ? (
              ...
              <>
                <h4 style={{margin: '0 0 5px 0'}}>{popupData.location.split(',')[0]}</h4>
                <p>Temp: {popupData.weather.temperature}°C</p>
                <p>Wind: {popupData.weather.windspeed} km/h</p>
              </>
            )}
          </div>
        )
...
// --- ResizeHandler (No changes needed) ---
const ResizeHandler = () => {
  const map = useMap();
  useEffect(() => {
    map.invalidateSize();
...
// --- getDisasterIcon (No changes needed) ---
const getDisasterIcon = (category) => {
  ...
  return L.divIcon({ className: 'disaster-icon', html: iconHtml, iconSize: [24, 24] });
};

// --- createClusterCustomIcon (No changes needed) ---
const createClusterCustomIcon = function (cluster) {
  const count = cluster.getChildCount();
  let size = 20;
  let clusterColor = '#92d400';
  if (count >= 10) { clusterColor = '#f5d423'; size = 20; }
  if (count >= 100) { clusterColor = '#f0962d'; size = 20; }
  return L.divIcon({ html: `<div style="background-color: ${clusterColor}; width: 100%; height: 100%; border-radius: 50%; border: 2px solid rgba(0,0,0,0.5);"></div>`, className: 'custom-marker-cluster-solid', iconSize: L.point(size, size, true) });
};

// --- Main Map2D Component (No changes needed) ---
function Map2D({ data }) {
  const position = [20, 0];
  return (
    <MapContainer center={position} zoom={2} className="map-container">
      ...
      <MarkerClusterGroup iconCreateFunction={createClusterCustomIcon}>
        {data.map(item => {
          if (!item || !item.reclat || !item.reclong) { return null; }
          return (
            <Marker key={item.id} position={[item.reclat, item.reclong]} icon={getDisasterIcon(item.category)}>
              <Popup><b>{item.name}</b></Popup>
            </Marker>
          );
        })}
      </MarkerClusterGroup>
